
# The Word **er** and **er-words**  *(B1)*

Pronominal adverb `er` is one of the most versatile — and tricky — words in Dutch.

It plays four distinct roles.

## Existential **er** (there is / there are)

Used to introduce a new, unspecified noun into the conversation.

| Dutch | English |
|-------|---------|
| Er is een probleem. | There is a problem. |
| Er zijn veel mensen op straat. | There are many people on the street. |
| Er was eens een koning. | Once upon a time there was a king. |
| Er is geen melk meer. | There is no milk left. |
| Er wordt hier niet gerookt. | Smoking is not allowed here. (lit. It is not smoked here.) |

> Always at or near the start of the sentence.
>
> This `er` cannot be omitted. Without it the sentence is ungrammatical: ~~*Is een probleem.*~~

## Locative **er** (there — referring to a place)

Replaces a place already mentioned. Equivalent to English "there".

| Dutch | English |
|-------|---------|
| Ik woon in Amsterdam. Ik woon er al tien jaar. | I've lived there for ten years. |
| Ben je in Parijs geweest? Ja, ik ben er vorig jaar geweest. | Have you been to Paris? Yes, I was there last year. |
| Ga je naar de markt? Ik ga er elke zaterdag naartoe. | Do you go to the market? I go there every Saturday. |

## Quantitative **er** (of them — with numbers)

When a counted noun is omitted (already known from context), `er` fills its place.

| Dutch | English |
|-------|---------|
| Hoeveel appels heb je? — Ik heb er drie. | How many apples do you have? — I have three (of them). |
| Er zijn er vijf over. | There are five (of them) left. |
| Ik ken er maar één. | I only know one (of them). |
| Ze heeft er veel gekocht. | She bought a lot of them. |

> Note: when `er` is existential AND quantitative together: *Er zijn er vijf.* The first `er` = existential, the second `er` = quantitative.

## Pronominal **er-words** (er + preposition)

This is the most important use. When referring back to a **thing** (not a person) after a preposition.

Dutch combines `er` with the preposition into a single word — called an **er-word** (pronominaladverbium).

**Formula:** `er` + preposition = er-word

| Preposition | Er-word | Example |
|-------------|---------|---------|
| op | *erop* | De sleutels liggen op tafel. Ze liggen **erop**.* — The keys are on it. |
| in | *erin* | Stop het in de tas. Stop het **erin**.* — Put it in it. |
| aan | *eraan* | Ik denk aan het plan. Ik denk **eraan**.* — I'm thinking about it. |
| over | *erover* | We praten over het probleem. We praten **erover**.* — We're talking about it. |
| voor | *ervoor* | Hij betaalt voor de tickets. Hij betaalt **ervoor**.* — He pays for them. |
| mee | *ermee* | Ik ben blij met het resultaat. Ik ben **ermee** blij.* — I'm happy with it. |
| door | *erdoor* | Ze rijden door de tunnel. Ze rijden **erdoor**.* — They drive through it. |
| uit | *eruit* | Hij stapt uit de auto. Hij stapt **eruit**.* — He gets out of it. |
| naast | *ernaast* | Ze woont naast de bakker. Ze woont **ernaast**.* — She lives next to it. |
| onder | *eronder* | Het ligt onder het bed. Het ligt **eronder**.* — It's under it. |
| tegen | *ertegen* | Ik ben tegen het idee. Ik ben **ertegen**.* — I'm against it. |
| af | *eraf* | Haal het label eraf.* — Take the label off (it). |
| omheen | *eromheen* | Er staat een hek omheen. Er staat een hek **eromheen**.* — There's a fence around it. |

> **Person vs. thing:** Er-words only replace **things**. For people, use the regular preposition + pronoun:
>
> - *Ik denk aan hem.* (I'm thinking about him.) — NOT ~~*ik denk eraan*~~ for a person
> - *Ik denk eraan.* (I'm thinking about it.) — for a thing ✓

## Splitting er-words

Er-words can — and often must — be **split** when other words (adverbs, negation) appear between `er` and the preposition.

| Unsplit | Split | English |
|---------|-------|---------|
| Ik denk eraan. | Ik denk er vaak **aan**. | I often think about it. |
| Hij zit erop. | Hij zit er al lang **op**. | He has been sitting on it for a long time. |
| Ze zijn erover blij. | Ze zijn er erg **over** tevreden. | They are very satisfied with it. |
| Ik ben ermee klaar. | Ik ben er bijna mee klaar. | I'm almost done with it. |

> The split form is extremely common in speech.
> Beginners often produce the unsplit form, which is grammatical but sounds bookish.
